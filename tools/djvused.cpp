...
int mylinecounter = 1;
int mylinetotalcounter = 1;
int mywordcounter = 1;
int mywordtotalcounter = 1;
	      ...
	      const GP<ByteStream> &out, int indent)
{
  ...
  GUTF8String pagemessage = "(bogus";
  if (strcmp(zone_names()[zinfo].name,"word")==0) {
      ...
      pagemessage.format(";file:%s?djvuopts=&page=%04d&highlight=%04d,%04d,%04d,%04d;%s p=%d l=%d tl=%d w=%d tw=%d; ※ ",
		       filename,
		       //		       zone_names()[zinfo].name,
		       mypagecounter,    
		       zone.rect.xmin, zone.rect.ymin, 
		       zone.rect.xmax-zone.rect.xmin, zone.rect.ymax-zone.rect.ymin,
		       filename, mypagecounter, mylinecounter, mylinetotalcounter, mywordcounter, mywordtotalcounter);
      out->write((const char*)pagemessage, pagemessage.length());
      print_c_string(data, length, *out, utf8);
      out->write("\n", 1);
//      fprintf(stdout,"file  %s\n", filename);
 ...
 if (strcmp(zone_names()[zinfo].name,"word")==0) {
   mywordcounter++;mywordtotalcounter++;}
 
 if (strcmp(zone_names()[zinfo].name,"line")==0) {
   mylinecounter++;mylinetotalcounter++;}
 
  // JSB word handling end
...
command_print_txt(ParsingByteStream &)
{
  ...
  { // extra nesting for windows
	...
	mylinecounter=0;
	mywordcounter=1;
	// pagemessage.format("[page %04d;file:%s?djvuopts=&page=%04d;%s; ※  lines total %d,  words total %d",
	// 	       mypagecounter,    
	// 	       filename,
	// 	       mypagecounter,    
	// 	       filename, mylinetotalcounter, mywordtotalcounter);
	// out->write((const char*)pagemessage, pagemessage.length());
	// out->write("\n", 1);
	// does not compile
	  }
  ...
  G_TRY
     {
      ...
      if (!g().djvufile)
        usage();
      // JSB
      filename = (const char*)ToNative(g().djvufile);
      //      fprintf(stdout,"file  %s\n",(const char*)ToNative(g().djvufile));
      // fprintf(stdout,"file  %s\n", filename);
      // JSB
      // BOM
#ifdef _WIN32
